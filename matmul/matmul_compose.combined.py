...
module matmul_{0}x{0}_systolic{1}(
  ...
  for i in range(num_of_bram):
    file.write("""
  wire [`MAT_MUL_SIZE*`DWIDTH-1:0] c_data_{0}_{1}_NC;
    """.format(i,num_of_bram-1))

  #instaniate building block matmul
  for i in range(num_of_bram):
    for j in range(num_of_bram):
      file.write('  /////////////////////////////////////////////////\n'
  		 '  // Matmul {0}_{1}\n'
      ...
      if(j != num_of_bram - 1):
        ...
        file.write('  wire [`MAT_MUL_SIZE*`DWIDTH-1:0] c_data_{a}_{b}_to_{c}_{d}_NC;\n'.format(a = i, b = j, c = i, d = j+1))
      ...
      file.write('matmul_slice u_matmul_{c}x{c}_systolic_{a}_{b}(\n'
  		 '  .clk(clk),\n'
  		 '  .reset(reset),\n'
  		 '  .pe_reset(pe_reset),\n'
  		 '  .start_mat_mul(start_mat_mul),\n'
